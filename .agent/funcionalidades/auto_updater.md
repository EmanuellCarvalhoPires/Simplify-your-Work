...
- **Arquivos e Componentes Envolvidos**:
  ...
  - [`src/components/common/UpdateModal.tsx`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/src/components/common/UpdateModal.tsx)
...
| :--- | :--- | :--- | :--- |
| `v1.4.1` | 2026-09-14 | Antigravity AI | **Correção de Flag no Workflow de Release**: Removida a flag `-c.publish.releaseType=release` que era erroneamente interpretada pelo `electron-builder` como caminho de arquivo de configuração (`ENOENT: .publish.releaseType=release`), utilizando a configuração declarativa `"releaseType": "release"` já existente no `package.json` e ajustando os caminhos de trigger do workflow para incluir `.github/workflows/**`. |
| `v1.4.0` | 2026-08-24 | Antigravity AI | **Correção de Publicação Automática de Releases (Remoção do modo Draft)**: 1) Adicionada a propriedade `"releaseType": "release"` em `build.publish` no [`package.json`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/package.json). 2) Atualizado o workflow [`.github/workflows/release.yml`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/.github/workflows/release.yml) com a flag `-c.publish.releaseType=release` e passo dedicado pós-build executando `gh release edit "$TAG" --draft=false --latest` para assegurar que cada release seja publicada imediatamente sem ficar retida como rascunho. |
| `v1.3.0` | 2026-08-21 | Antigravity AI | **Buscador Automático de Atualizações na Inicialização & Modal de Confirmação**: Criado o componente [`UpdateModal.tsx`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/src/components/common/UpdateModal.tsx) e adicionada verificação suave 2.5s após inicializar o app em [`App.tsx`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/src/App.tsx). Quando há nova versão no GitHub Releases, abre um modal elegante perguntando se o usuário deseja atualizar agora ou lembrar mais tarde, com barra de progresso em tempo real e botão de reinicialização após download. |
| `v1.2.0` | 2026-08-21 | Antigravity AI | **Controle Manual Estrito e Proteção contra Fechamento Inesperado**: Desativado `autoInstallOnAppQuit` e removida verificação automática agressiva em timer que provocava o fechamento do app após inicialização. Adicionados handlers de proteção global de exceções (`uncaughtException`, `unhandledRejection`) em [`electron/main.ts`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/electron/main.ts). |
| `v1.1.0` | 2026-08-21 | Antigravity AI | **Sanitização de Erros e Tratamento de Repositórios Sem Release**: Implementado `formatUpdaterError` em [`electron/updater-service.ts`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/electron/updater-service.ts) e banner amigável em [`SettingsView.tsx`](file:///c:/Users/Trabalho/Documents/Simplify%20your%20Work/src/components/settings/SettingsView.tsx), tratando adequadamente retornos 404 (quando ainda não há Releases publicadas no GitHub) sem despejar payloads brutos de HTTP na interface. |
