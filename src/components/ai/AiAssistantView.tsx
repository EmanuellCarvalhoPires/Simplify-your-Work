...
export const AiAssistantView: React.FC<AiAssistantViewProps> = ({
  ...
  const providerInfo = AI_PROVIDERS[provider] || AI_PROVIDERS.chatgpt;
  const currentUrl = providerInfo.url;
  ...
  // Quando a URL do provedor mudar, atualiza
  useEffect(() => {
    ...
    if (webviewRef.current && currentUrl) {
      ...
      try {
        ...
        // Ignora caso webview ainda esteja inicializando
      }
    }
  }, [currentUrl]);
  ...
  useEffect(() => {
    ...
    // Safety timeout: Garante que o spinner inicial nunca fique preso indefinidamente
    ...
    const handleStopLoading = () => {
      ...
      try {
        ...
        // Ignora caso método não esteja pronto
      }
    };
    ...
    const handleFailLoad = (e: any) => {
      ...
      setIsInitialLoad(false);
      setHasError(true);
      setErrorMessage(e.errorDescription || `Falha ao carregar a página de ${providerInfo.name}.`);
    };
  ...
  const handleOpenGoogleAuth = async () => {
    if (window.electronAPI?.openGoogleAuthWindow) {
      try {
        const res = await window.electronAPI.openGoogleAuthWindow(provider, currentUrl);
        if (res?.success) {
          handleReload();
        }
      } catch (e) {
        console.error('Erro ao abrir janela de autenticação Google:', e);
      }
    } else {
      handleOpenExternal();
    }
  ...
  const renderIcon = () => {
    if (provider === 'gemini' || provider === 'claude') {
      return <Sparkles size={15} color={providerInfo.color} />;
    }
    return <Bot size={15} color={providerInfo.color} />;
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.toolbar}>
        <div style={styles.navGroup}>
          ...
          <button onClick={() => handleNavigate(currentUrl)} style={styles.toolButton} title="Página Inicial">
            <Home size={16} />
          </button>
        </div>
        ...
        {/* Informação da IA ativa */}
        <div style={styles.shortcutsGroup}>
          <div style={styles.aiActiveBadge}>
            {renderIcon()}
          ...
          {/* Botão de Autenticação Segura Google para o Gemini */}
          {provider === 'gemini' && (
            <button
              ...
              title="Abre uma janela nativa segura para autenticar sua conta Google e salvar a sessão no app"
            >
              <LogIn size={14} />
              <span>Login Google (Janela Segura)</span>
            </button>
          )}
        </div>
        ...
        {isInitialLoad && !hasError && (
          <div style={styles.initialLoadingOverlay}>
            <div style={styles.initialLoadingCard}>
              <Loader2 size={36} className="spin-icon" color={providerInfo.color} />
              <span style={styles.initialLoadingText}>
                Carregando {providerInfo.name}...
              </span>
            </div>
          </div>
        )}
        ...
        {hasError ? (
          <div style={styles.errorContainer}>
            <div style={styles.errorCard}>
              ...
              <h3 style={styles.errorTitle}>Não foi possível carregar {providerInfo.name}</h3>
